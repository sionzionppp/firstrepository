...
public class PagingUtilAjax {
	...
	/**
	 * @param url 			: 페이징 적용 대상 주소  (예) 페이지 이동이 없기 때문에 url이 필요없다.
	 * @param currentPage 	: 현재 페이지
	 * @param totRecord 	: 젠체 게시물수
	 * @param blockCount 	: 한 블럭의 게시물 수
	 * @param blockPage  	: 한화면에 보여질 블럭 수
	 **/
	public PagingUtilAjax(int currentPage, int totRecord, int blockCount, int blockPage) {
		...
		if (currentPage > blockPage) {
			pagingHtml.append("<a href='javascript:goPaging(" + (startPage - 1) + ")'>");
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (i == currentPage) {
				...
				pagingHtml.append(" <a href='javascript:goPaging(");
				pagingHtml.append(i);
				pagingHtml.append(")'>");
				pagingHtml.append(i);
				pagingHtml.append("</a>");
		...
		if (maxPage - startPage >= blockPage) {
			pagingHtml.append("<a href='javascript:goPaging(" + (endPage + 1) + ")'>");
